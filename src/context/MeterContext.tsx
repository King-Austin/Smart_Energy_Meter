...
export const MeterProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const toggleAutoTopup = () => {
    setMeterData(prev => {
      const next = !prev.auto_topup_enabled;
      addNotification(
        'Auto-Recharge Updated',
        next ? 'Auto-recharge enabled for low balance.' : 'Auto-recharge disabled.',
        'system'
      );
      return { ...prev, auto_topup_enabled: next };
    });
  };

  const setAutoTopupThreshold = (threshold: number) => {
    setMeterData(prev => ({ ...prev, auto_topup_threshold: threshold }));
  };

  // Live Telemetry Sync Engine
  useEffect(() => {
    const interval = setInterval(async () => {
      // 1. If in Live API Mode, fetch real telemetry from configured endpoint
      ...
      // 2. Otherwise run internal high-fidelity simulation engine
      setMeterData(prev => {
        ...
        const basePower = prev.main_supply_connected ? 2.46 : 0;
        const newActivePower = Math.max(0.1, Number((basePower + jitterPower).toFixed(2)));
        ...
        const newCurrent = Number(((newActivePower * 1000) / (newVoltage * newPf)).toFixed(1));
      ...
      setActiveSession(prevSession => {
        ...
        const currentPower = Math.max(50, prevSession.power_limit_w + powerJitter);
        ...
        const isTimeLimitReached = newElapsed >= prevSession.duration_limit_seconds;
      ...
      setReceivingSession(prevRec => {
        ...
        if (newReceived >= prevRec.energy_limit_kwh || newElapsed >= prevRec.duration_limit_seconds) {
  ...
  const startSharing = (
    ...
    if (meterData.device_status === 'offline') {
      return { success: false, error: 'Your Meter must be online to share energy.' };
    }
    ...
    addNotification(
      'Sharing Started',
      `Energy sharing with ${recipient.meter_name} has started.`,
      'sharing'
    );
    return { success: true };
  };
  ...
  const toggleMeterOnline = (forcedOnline?: boolean) => {
    setMeterData(prev => {
      ...
      if (!isOnline) {
        ...
        addNotification('Meter Offline', 'My Home Meter has lost its internet connection.', 'offline');
      } else {
        addNotification('Meter Online', 'My Home Meter reconnected to cloud.', 'system');
      }
